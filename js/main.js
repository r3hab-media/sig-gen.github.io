document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("socialMediaChoice1")
    .addEventListener("change", toggleSocialSection);
  document
    .getElementById("socialMediaChoice2")
    .addEventListener("change", toggleSocialSection);

  function toggleSocialSection(event) {
    const socialMediaSection = document.querySelector("#socialMediaSection");
    socialMediaSection.style.display =
      event.target.value === "socialMediaDefault" ? "none" : "flex";
  }

  function generateSignatures() {
    //social links
    const socialChoice = document.querySelector(
      'input[name="socialMediaChoice"]:checked'
    ).value;
    ...
    const deptUrl =
      document.getElementById("deptUrl").value || "https://scottsdaleaz.gov";
    const deptUrlDisplay =
      document.getElementById("deptUrlDisplay").value || "City of Scottsdale";
    ...
    let socialLinks = "";
    if (socialChoice === "socialMediaDefault") {
      socialLinks = `<a href="https://www.scottsdaleaz.gov/news/social-media">Connect with us on social media</a>`;
    } else {
      if (facebook)
        socialLinks += `<a href="${facebook}" class="sig__social--elem">Facebook</a>`;
      if (twitter)
        socialLinks += ` | <a href="${twitter}" class="sig__social--elem">Twitter</a>`;
      if (instagram)
        socialLinks += ` | <a href="${instagram}" class="sig__social--elem">Instagram</a>`;
      if (youtube)
        socialLinks += ` | <a href="${youtube}" class="sig__social--elem">YouTube</a>`;
    }
    ...
    const template3 = `
      <div class="sig card">
        <div class="card-body">
          <div class="sig__name">${name}${pnLettersTrue}${
      pronoun ? `<span class="sig__pronoun">(${pronoun})</span>` : ""
    }</div>
          ...
          <a href="${deptUrl}" class="sig__link">${deptUrlDisplay}</a>
